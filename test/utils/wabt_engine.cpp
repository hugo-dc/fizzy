    ...
    WasmEngine::FuncRef func_ref, const std::vector<uint64_t>& args)
{
    ...
    // This is not very smart design. To mitigate the overhead, we set sizes to much lower values
    // than default ones.
    wabt::interp::Thread::Options thread_options;
    thread_options.value_stack_size = 1024;
    thread_options.call_stack_size = 1024;
