    ...
    const Instance& instance, ExternalKind kind, std::string_view name)
{
    ...
    return (it == instance.module.exportsec.end() ? std::nullopt : std::make_optional(it->index));
}
...
std::optional<ExternalGlobal> find_exported_global(Instance& instance, std::string_view name)
{
    ...
    else
    {
        ...
        return ExternalGlobal{
            &instance.globals[global_idx], instance.module.globalsec[global_idx].is_mutable};
    }
}
